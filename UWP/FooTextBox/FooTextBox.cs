...
namespace FooEditEngine.UWP
{
    ...
    public sealed class FooTextBox : Control,IDisposable
    {
...
#endif
        Windows.ApplicationModel.SuspendingDeferral suspedning = null;
        ...
        public FooTextBox()
        {
            ...
            this.TextStoreHelper = new TextStoreHelper(this._Controller);
            ...
            {
                _Document.LayoutLines.ClearLayoutCache();
            };
            Application.Current.Suspending += async (s, e) =>
            {
                suspedning = e.SuspendingOperation.GetDeferral();
                await Dispatcher.RunIdleAsync((ie) => {
                    if (suspedning != null)
                    {
                        suspedning.Complete();  //中断中にレタリングをすると落ちるので、完了まで中断を遅らせる
                    }
                });
                this.timer.Stop();
            };
        ...
        public int GetIndexFromPostion(Windows.Foundation.Point p)
        {
            ...
            return (int)this._View.GetIndexFromLayoutLine(tp);
        }
        ...
        /// <param name="p">座標</param>
        /// <returns>インデックスを返す</returns>
        public long GetLongIndexFromPostion(Windows.Foundation.Point p)
        {
            if (this.Document.FireUpdateEvent == false)
                throw new InvalidOperationException("");
            TextPoint tp = this._View.GetTextPointFromPostion(p);
        ...
        private void Document_LoadProgress(object sender, ProgressEventArgs e)
        {
            ...
            else if(e.state == ProgressState.Complete)
            {
                ...
                {
                    var lineLength = this.Document.LayoutLines.GetLengthFromLineNumber(0);
...
　      /// <inheritdoc/>
        ...
        private void TextEditContext_TextUpdating(CoreTextEditContext sender, CoreTextTextUpdatingEventArgs args)
        {
            ...
            TextStoreHelper.SetSelectionIndex(this._View, sel.StartCaretPosition, sel.EndCaretPosition);
            TextStoreHelper.InsertTextAtSelection(args.Text, args.Range.StartCaretPosition, args.Range.EndCaretPosition, isTip);
            this.Refresh();
        ...
        void InheritanceDependecyPropertyCallback(DependencyObject sender, DependencyProperty dp)
        {
            ...
            {
                this.Document.RightToLeft = this.FlowDirection == Windows.UI.Xaml.FlowDirection.RightToLeft;
                if (this.horizontalScrollBar != null)
